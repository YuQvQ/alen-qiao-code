// circle_fit.h — 点集最小二乘圆拟合
// 输入：points(Point2DList), image(可选,用于绘制)
// 输出：result_image(Image), center(Pose2D), radius(Double), residuals(Double), count(Int)
#pragma once
#include "algo_base.h"
...
class CircleFitAlgo : public AlgoBase {
public:
...
private:
    ...
    double out_cx_=0, out_cy_=0, out_radius_=0, out_residuals_=0;
    int out_count_=0;
    // 绘制参数
